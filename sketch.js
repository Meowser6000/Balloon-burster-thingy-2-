...
function draw() {
  ...
  if(end == true) {
    if(arrow !== undefined) {
      ...
      greenB.setVisibleEach(false);
      redB.setVisibleEach(false);
      pinkB.setVisibleEach(false);
      blueB.setVisibleEach(false);
      background(0);
      textSize(20);
      fill("white");
      text("You lose! your score is " +score,100,200);
  } else {
    background(255);
    // moving ground
    scene.velocityX = -3 
    ...
    if(green !== undefined && green.x > 291) {
      end = true;
    }


    if(blue !== undefined && blue.x > 291) {
      end = true;
    }


    if(red !== undefined && red.x > 291) {
      end = true;
    }


    if(pink !== undefined && pink.x > 291) {
      end = true;
    }
